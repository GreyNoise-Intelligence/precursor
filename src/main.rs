...
use clap::{Arg, ArgAction, Command, ColorChoice, ArgMatches, builder::PathBufValueParser, value_parser};
use dashmap::DashMap;
use hex;
...
fn  handle_line(line: &String, 
    ...
    if let Some(payload_key) = args.get_one::<String>(INPUT_JSON_KEY) {
        ...
        match out.next() {
            ...
            Some(Err(e)) => {
                eprintln!("Unable to parse JSON pattern: {:?} with error: {:?}", payload_key, e);
            },
            None => {
                ...
                for name in re.capture_names() {
                    if let Some(name) = name {
                        if caps.name(name).is_some() {
                            ...
                            let mut count = counter_pcre_matches.entry(name.to_string()).or_insert(0);
                            *count += 1;
    ...
    if match_exists.lock().unwrap().clone() {
        ...
        */
        ...
        json_clone["tags"] = matched_capture_groups.lock().unwrap().clone();
         ...
         payload_reports.lock().unwrap().insert(xxh3_64_sum_string, json_clone);
    }
}